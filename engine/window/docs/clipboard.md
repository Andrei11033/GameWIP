@page window_clipboard Clipboard and data transfer

`GameWIP::Window::Clipboard` is a synchronous stateless service for the
operating-system clipboard. Include `window/clipboard.h`; it includes the shared
`window/data_transfer.h` vocabulary. Both headers are opt-in parts of the
existing `GameWIP::Window` package and are not included by `window/window.h`.
...
GitHub issue 52 owns `window/data_transfer.h`. The future issue 58 native drag-and-drop feature
