...
Window deterministic failure/state hooks are source-tree-only and are enabled with `WINDOW_ENABLE_TEST_HOOKS`, which defines
`WINDOW_INTERNAL_TEST_HOOKS` for repository validation targets.

`window/internal/window_test_hooks.h` is not installed and is not a supported consumer header. Installed package validation explicitly checks that
`WINDOW_INTERNAL_TEST_HOOKS` does not leak through `GameWIP::Window`.
