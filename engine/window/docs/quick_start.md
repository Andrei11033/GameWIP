...
Use explicit `close()` when cleanup errors matter. The destructor is `noexcept` and best effort. Destroying an open Window from a non-owning thread violates the ownership contract; arrange explicit close on its opening thread.
...
- @ref window_native_interop and @ref window_renderer_feedback define the two narrow Renderer integration seams.
