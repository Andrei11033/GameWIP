...
attached. Include `window/renderer_bridge.h`, check `Renderer::hasOcclusionProvider(window)` for attachment state, and call
...
Include `window/display_info.h` and perform the first `Display::getColorInfo(...)` query on the Window owner thread if that thread should receive
