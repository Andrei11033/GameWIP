...
OS monitor color facts are not renderer feedback. Include `window/display_info.h` and call:

```cpp
const auto monitorColor = GameWIP::Window::Display::getColorInfo(monitor);
const auto windowColor = GameWIP::Window::Display::getColorInfo(window);
```
