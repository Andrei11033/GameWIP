...
namespace GameWIP
{
    ...
    WindowResult Window::restoreWindowedPlacement()
    {
        ...
        RECT requestedWindowRect{0, 0, nativeWindow->requestedClientWidth, nativeWindow->requestedClientHeight};

        nativeWindow->dpi = getWindowDpi(nativeWindow->handle);
        if (!adjustWindowRectForDpi(requestedWindowRect, nativeWindow->windowedStyle, nativeWindow->windowedExtendedStyle, nativeWindow->dpi))
        {
            return recordResult(WindowResult::PlatformCallFailed, GetLastError());
        }
    ...
    Window::FullscreenModeSnapshot Window::captureFullscreenModeSnapshot() const
    {
        ...
        std::copy(std::begin(nativeWindow->fullscreenDeviceName), std::end(nativeWindow->fullscreenDeviceName), std::begin(snapshot.deviceName));
    ...
    WindowResult Window::restoreFullscreenModeSnapshot(const FullscreenModeSnapshot &snapshot)
    {
        ...
        if (!snapshot.suspended)
        {
            ...
            LONG displayResult = ChangeDisplaySettingsExW(snapshot.deviceName, &fullscreenMode, nullptr, CDS_FULLSCREEN, nullptr);
            if (displayResult != DISP_CHANGE_SUCCESSFUL)
            {
                return WindowResult::ModeChangeFailed;
            }
        ...
        std::copy(std::begin(snapshot.deviceName), std::end(snapshot.deviceName), std::begin(nativeWindow->fullscreenDeviceName));
    ...
    {
        outTarget = {};
        outTarget.monitor = nativeWindow->hasRequestedFullscreenDisplayMode ? static_cast<HMONITOR>(nativeWindow->requestedFullscreenMonitor.handle)
        ...
        std::copy(std::begin(outTarget.monitorInfo.szDevice), std::end(outTarget.monitorInfo.szDevice), std::begin(outTarget.deviceName));
        ...
        const FullscreenModeSnapshot &previousState)
    {
        if (target.savedModeIsFromDifferentDevice && !previousState.suspended)
        {
            ...
            LONG restorePreviousDisplayResult = ChangeDisplaySettingsExW(previousState.deviceName, &previousSavedDisplayMode, nullptr, 0, nullptr);
    ...
    {
        std::copy(std::begin(target.deviceName), std::end(target.deviceName), std::begin(nativeWindow->fullscreenDeviceName));
