...
namespace GameWIP
{
    ...
    {
        return nativeWindow != nullptr && nativeWindow->suppressStartupEvents && isStartupStateEvent(type);
    }
    ...
    WindowResult Window::create(const WindowDescription &description)
    {
        ...
        nativeWindow->handle = CreateWindowExW(
            extendedWindowStyle,
            className,
            nativeWindow->utf16Scratch.c_str(),
            windowStyle,
            CW_USEDEFAULT,
            CW_USEDEFAULT,
            outerWidth,
    ...
    void Window::pollEvents(Input::InputState &inputState, Input::InputDeviceRegistry &inputDevices)
    {
        ...
        while (PeekMessageW(&message, nullptr, 0, 0, PM_REMOVE))
        {
            ...
            bool clearInputAfterDispatch = nativeWindow != nullptr && message.hwnd == nativeWindow->handle && message.message == WM_KILLFOCUS;
    ...
    void Window::setMinClientSize(int width, int height)
    {
        if (nativeWindow != nullptr)
        {
            ...
            nativeWindow->requestedClientWidth =
                clampClientDimension(nativeWindow->requestedClientWidth, nativeWindow->minClientWidth, nativeWindow->maxClientWidth);
            nativeWindow->requestedClientHeight =
                clampClientDimension(nativeWindow->requestedClientHeight, nativeWindow->minClientHeight, nativeWindow->maxClientHeight);
        }
    }
    ...
    WindowInfo Window::getInfo() const
    {
        ...
        return WindowInfo{.instance = nativeWindow->instance, .handle = nativeWindow->handle};
    }
