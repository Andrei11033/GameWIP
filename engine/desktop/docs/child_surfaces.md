@page desktop_child_surfaces Native child surfaces

`GameWIP::Desktop::ChildSurface` is an opt-in managed native child host inside one open top-level `Window`. It exists for browser views, native
...
Include `desktop/child_surface.h` and link the existing `GameWIP::Desktop` target. The normal `desktop/window.h` header does not include this optional
surface.
...
Win32 consumers include `desktop/native/win32.h` and call `Native::Win32::getHandle(surface)`. The returned HWND is borrowed and valid only while native
