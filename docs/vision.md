...
1. **Simulation first, visuals second**
   Graphics should support development and debugging early. A full visual engine is not the first priority.

2. **Simple first, deep later**
   Every major system should have a simple usable version before advanced configuration is added.

3. **Shared structural backbone**
   Vehicles, buildings, and destructible structures should use the same core structural model where possible.

4. **Damage should matter**
   Destruction should not only remove blocks visually. It should affect strength, mass, connectivity, components, and behavior.

5. **Only selected systems need very high tick rates**
   High-frequency simulation should be reserved for systems that actually need it, such as control loops, guidance, stabilization, sensors, and weapons.
