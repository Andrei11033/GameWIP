...
/// @brief Private TestSupport correctness cases grouped by behavioral responsibility.
    ...
    void testUtf8TextFileContracts(TestSupport::Context &context, const std::filesystem::path &root)
    {
        ...
        static_cast<void>(
            context.expectEq("malformed text write preserves destination", std::string("sentinel"), malformedPreservedRead.text));
        ...
        static_cast<void>(
            context.expectEq("incomplete text write preserves destination", std::string("sentinel"), incompletePreservedRead.text));

        const std::filesystem::path malformedParent = root / "utf8" / "malformed-not-created";
        const TestSupport::Types::InfrastructureStatus malformedNoSideEffect =
            TestSupport::writeTextFile(malformedParent / "bad.txt", malformed);
        static_cast<void>(context.expectEq(
        ...
        const TestSupport::Types::InfrastructureStatus incompleteNoSideEffect =
            TestSupport::writeTextFile(incompleteParent / "bad.txt", incomplete);
        static_cast<void>(context.expectEq(
