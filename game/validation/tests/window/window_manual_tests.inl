/// @file window_manual_tests.inl
/// @brief Optional manual Window validation helpers and cases.
...
public:
    ...
    void refresh(const Window::Window &window)
    {
        ...
        const Window::Types::ContentScale scale = window.contentScale();
        const Window::Types::Dpi dpi = window.effectiveDpi();
        const Window::Types::Controls controls = window.controls();
        const Window::Types::FullscreenInfo fullscreen = window.fullscreenInfo();
        const ManualNativeWindowState native = manualNativeWindowState(window);
    ...
    ManualSurfaceLayout surfaceLayout = ManualSurfaceLayout::Standard)
{
    ...
    {
        const Window::Types::Events::PumpResult pump = Window::Events::wait(std::chrono::milliseconds{50});
    ...
    promptThread.join();
    const Window::Types::Events::PumpResult finalPump = Window::Events::poll();
...
void pumpManualPreparation(std::chrono::milliseconds duration)
{
    ...
    while (std::chrono::steady_clock::now() < deadline)
        static_cast<void>(Window::Events::wait(std::chrono::milliseconds{50}));
}
...
void testManualFilesAndShell(TestSupport::Context &context, const GameWIP::Test::WindowTestOptions &options)
{
    ...
    if (capabilities.supports(Window::Types::Capability::FileDrop))
    {
        const auto runDrop = [&](std::string_view name, std::string_view instruction, std::size_t minimumPaths)
        {
            ...
            const auto observeDrops = [&]
            {
                ...
                while (window.popEvent(event))
                {
                    if (const auto *drop = event.getIf<Window::Types::Events::FilesDropped>())
                    {
                        ++groupedEvents;
        ...
        if (disabledAnswer == TestSupport::Types::Reporting::ManualAnswer::Yes)
            static_cast<void>(
                context.expectFalse("disabled file drops queue no event", consumeEventOfType<Window::Types::Events::FilesDropped>(window)));
    }
    else
    {
    ...
    for (const bool resizable : {false, true})
    {
        for (const bool maximizable : {false, true})
        {
            ...
            Window::Types::Controls controls = window.controls();
    ...
    static_cast<void>(window.setResizable(true));
    Window::Types::Controls maximizableControls = window.controls();
    ...
    Window::Types::Controls independentControls = window.controls();
...
void testManualHdrAndAdvancedColor(TestSupport::Context &context, const GameWIP::Test::WindowTestOptions &options)
{
    ...
    for (const Window::Types::Display::Info &monitor : monitors.monitors)
    {
        const Window::Types::Display::ColorInfoResult direct = Window::Display::getColorInfo(monitor.id);
        ...
        static_cast<void>(context.expectTrue(
            ...
            info.hdrEnabled || info.activeColorSpace != Window::Types::Display::ColorSpace::Hdr10Pq));
        static_cast<void>(context.expectTrue(
            "HDR classification is truthful",
            !info.hdrEnabled || info.activeColorSpace == Window::Types::Display::ColorSpace::Hdr10Pq ||
                info.activeColorSpace == Window::Types::Display::ColorSpace::Unknown));
        recordManualCheck(
            context,
            window,
            "display-color facts",
    ...
    const Window::Types::Display::ColorInfoResult windowInfo = Window::Display::getColorInfo(window);
    ...
    {
        static_cast<void>(context.expectTrue("HDR toggle remains queryable", Window::Display::getColorInfo(window).status.ok()));
        static_cast<void>(context.expectTrue(
