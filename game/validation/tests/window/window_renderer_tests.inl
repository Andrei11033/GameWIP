/// @file window_renderer_tests.inl
...
void testPointerHitMask(TestSupport::Context &context)
{
    ...
    namespace FeedbackTypes = Window::Types::Renderer;
    ...
    static_cast<void>(
        context.expectFalse("unused Window has no renderer-integration sidecar", Window::TestHooks::hasRendererIntegrationState(window)));
    ...
    static_cast<void>(
        context.expectFalse("unsupported renderer operation remains allocation-free", Window::TestHooks::hasRendererIntegrationState(window)));
    Window::TestHooks::enablePointerHitMaskBridge(window);
    static_cast<void>(
        context.expectTrue("renderer sidecar is allocated on first enabled use", Window::TestHooks::hasRendererIntegrationState(window)));
    ...
    static_cast<void>(context.expectEq("failed first publication keeps no mask", std::size_t{0}, Window::TestHooks::pointerHitMaskWordCount(window)));
    ...
    static_cast<void>(
        context.expectEq("mask stores first physical pixel", MaskWord{1}, Window::TestHooks::pointerHitMaskWord(window, 0) & MaskWord{1}));
    static_cast<void>(context.expectTrue(
        "mask stores last physical pixel",
        (Window::TestHooks::pointerHitMaskWord(window, lastWord) & (MaskWord{1} << lastBit)) != 0));
    static_cast<void>(
        context.expectTrue("first logical position samples the first interactive pixel", Window::TestHooks::pointerHitMaskAccepts(window, {0, 0})));
    static_cast<void>(
        context.expectTrue("row-local mask lookup samples an independently padded row", Window::TestHooks::pointerHitMaskAccepts(window, rowSample)));
    static_cast<void>(
    ...
    static_cast<void>(context.expectEq("stale publication preserves active data", MaskWord{2}, Window::TestHooks::pointerHitMaskWord(window, 0)));
    ...
    static_cast<void>(
        context.expectEq("movement preserves active mask revision", newer.target.generation, Window::TestHooks::pointerHitMaskGeneration(window)));
