...
`getHealth()` is separate and coherent. `Types::Health::Snapshot` contains current `Healthy`, `Degraded`, or `Disabled` state, effective normal output, compact failure source, portable IO error, native code, and the failure count for the current initialization epoch.
