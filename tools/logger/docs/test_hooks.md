...
Logger validation hooks are available only when `LOGGER_INTERNAL_TEST_HOOKS` is enabled. They provide one-shot file open/write/flush failures, queue-copy allocation failure, fatal-popup failure, deterministic timed-flush timeout, and lifecycle/worker coordination pauses.
