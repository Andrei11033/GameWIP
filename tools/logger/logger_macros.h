...
/// @brief Lazy logger macro body. Captures the source once and evaluates message/format arguments only after Logger::shouldLog() passes.
/// @note Macro-filtered skips do not increment Logger::Stats::droppedFiltered; direct formatted logger calls still do.
...
#define LOGGER_TRACE(source, ...) LOGGER_DETAIL(::GameWIP::Logger::Level::Trace, trace, source, __VA_ARGS__)
#else
...
#define LOGGER_DEBUG(source, ...) LOGGER_DETAIL(::GameWIP::Logger::Level::Debug, debug, source, __VA_ARGS__)
#else
...
#define LOGGER_INFO(source, ...) LOGGER_DETAIL(::GameWIP::Logger::Level::Info, info, source, __VA_ARGS__)
/// @brief Lazy Warn logging. Message/format arguments are evaluated only when current filters allow Warn.
#define LOGGER_WARN(source, ...) LOGGER_DETAIL(::GameWIP::Logger::Level::Warn, warn, source, __VA_ARGS__)
/// @brief Lazy Error logging. Message/format arguments are evaluated only when current filters allow Error.
#define LOGGER_ERROR(source, ...) LOGGER_DETAIL(::GameWIP::Logger::Level::Error, error, source, __VA_ARGS__)
/// @brief Lazy Fatal logging. Message/format arguments are evaluated only when current filters allow Fatal. Does not terminate.
#define LOGGER_FATAL(source, ...) LOGGER_DETAIL(::GameWIP::Logger::Level::Fatal, fatal, source, __VA_ARGS__)
