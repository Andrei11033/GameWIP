...
The suite is one logical Assert module. Private behavior cases are grouped into focused `.inl` fragments for macro behavior, diagnostics, hooks,
...
tests use deterministic hook or environment paths and do not open real dialogs.

Real Win32 UI is manual and runtime opt-in. It covers Ignore Once, Always Ignore, Break with a debugger, Abort in a child, primary-dialog fallback,
and fallback default behavior.

## Hook coverage

With `ASSERT_INTERNAL_TEST_HOOKS=1`, tests can force primary action-dialog fallback, fallback behavior, debugger-attached results, and popup
suppression. State must be reset after every scenario.
