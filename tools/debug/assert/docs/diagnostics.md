...
On Win32, popup text is converted strictly from UTF-8 to UTF-16 through the Unicode library at the native boundary. Popup length limits are then
applied without separating a UTF-16 surrogate pair, so native truncation cannot split a Unicode scalar.
