...
if(NOT EXISTS "${compiler_runtime}")
    message(FATAL_ERROR
        "GNU compiler runtime does not exist: ${compiler_runtime}"
    )
endif()
...
file(GLOB source_dlls
    LIST_DIRECTORIES FALSE
    "${SOURCE_OUTPUT_DIR}/*.dll"
)
if(NOT source_dlls)
    message(FATAL_ERROR
        "No application DLLs were available in ${SOURCE_OUTPUT_DIR}."
    )
endif()
foreach(source_dll IN LISTS source_dlls)
    ...
    file(COPY_FILE
        "${source_dll}"
        "${VALIDATION_ROOT}/${source_dll_name}"
    )
endforeach()
...
if(NOT copy_result EQUAL 0)
    message(FATAL_ERROR
        "Runtime dependency staging failed (${copy_result}).\n"
        "stdout:\n${copy_output}\n"
        "stderr:\n${copy_error}"
    )
endif()
...
if(NOT staged_runtime_hash STREQUAL compiler_runtime_hash)
    message(FATAL_ERROR
        "The staged libstdc++-6.dll did not come from the active compiler."
    )
endif()
...
if(NOT launch_result EQUAL 0)
    message(FATAL_ERROR
        "The staged executable failed to launch (${launch_result}).\n"
        "stdout:\n${launch_output}\n"
        "stderr:\n${launch_error}"
    )
endif()
